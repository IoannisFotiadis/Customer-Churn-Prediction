#ustomer churn occurs when customers or subscribers stop doing business with a company or service, also known as customer attrition. It is also referred as loss of clients or customers. One industry in which churn rates are particularly useful is the telecommunications industry, because most customers have multiple options from which to choose within a geographic location.

#We are going to predict customer churn using telecom dataset(it can be found here: https://www.ibm.com/communities/analytics/watson-analytics-blog/guide-to-sample-datasets/). We will introduce Logistic Regression, Decision Tree, and Random Forest.

#Install packages
list.of.packages <- c("corrplot", "Rcpp", "ggthemes", "caret", "MASS", "randomForest", "party")
...
# Read our dataset
churn <- read.csv('C:/users/Ioannis/Downloads/Telco-Customer-Churn.csv')
str(churn)
...
#We’ll use all other columns as features to our model.
...
#We found that there are 11 missing values in "TotalCharges" columns. So, let's remove these rows with missing values.
 
sapply(churn, function(x) sum(is.na(x))) 

churn <- churn[complete.cases(churn), ]

# Here we should change “No internet service” to “No” for six columns, they are:
...
#All categorical variables have a reasonable broad distribution, therefore, all of them will be kept
...
#  1. The top three most-relevant features include Contract, Paperless Billing and tenure group, 
#all of which are categorical variables.
...
#One of the interesting perfomance measurements in logistic regression is Odds Ratio.Basically, 
#Odds retios is what the odds of an event is happening?
...
#On the other hand, if a customer is in a month-to-month contract, and in the tenure group of 0-12 months,
#and using PaperlessBilling, then this customer is more likely to churn. 
